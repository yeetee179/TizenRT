...
static T_APP_RESULT bt_stack_le_coc_callback(uint8_t coc_msg_type, void *coc_data)
{
	...
	case GAP_COC_MSG_LE_RECEIVE_DATA: {
		...
		printf("[######## %s : %d][GAP_COC_MSG_LE_RECEIVE_DATA] conn_handle: %d, cid: 0x%x, value_len:%d p_data:%x p_data:%x p_data:%x p_data:%x p_data:%x p_data:%x p_data:%x p_data:%x p_data:%x p_data:%x\r\n",
				__FUNCTION__, __LINE__,
				conn_handle, 
				recv_data->cid, 
				recv_data->value_len, 
				...
				recv_data->p_data[8],
				recv_data->p_data[9]);
		// p_evt = rtk_bt_event_create(RTK_BT_LE_GP_GAP, RTK_BT_LE_GAP_EVT_COC_RECEIVE_DATA_IND,0);
		...
		printf("[######## %s : %d] p_evt->group %d p_evt->evt %d p_evt->datalen %d p_evt->data %x \r\n",
				__FUNCTION__, __LINE__,
				p_evt->group, p_evt->evt, p_evt->data_len, p_evt->data);
		...
		printf("[######## %s : %d] %x\n", __FUNCTION__, __LINE__, (uint8_t *)data_ind + sizeof(rtk_bt_le_coc_receive_data_ind_t));
		printf("[######## %s : %d] data_ind->data %x\n", __FUNCTION__, __LINE__, data_ind->data);

		printf("[######## %s : %d] sizeof(rtk_bt_le_coc_receive_data_ind_t) %x\n", __FUNCTION__, __LINE__, sizeof(rtk_bt_le_coc_receive_data_ind_t));
		printf("[######## %s : %d] data_ind %x\n", __FUNCTION__, __LINE__, data_ind);
		printf("[######## %s : %d] data_ind->conn_handle %x\n", __FUNCTION__, __LINE__, &(data_ind->conn_handle));
		printf("[######## %s : %d] data_ind->cid %x\n", __FUNCTION__, __LINE__, &(data_ind->cid));
		printf("[######## %s : %d] data_ind->len %x\n", __FUNCTION__, __LINE__, &(data_ind->len));
		printf("[######## %s : %d] data_ind->data %x\n", __FUNCTION__, __LINE__, &(data_ind->data));
