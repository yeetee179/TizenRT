...
static inline int print_decimal_num(unsigned int unum, int radix, char padc, int padn)
{
...
loop:
			...
			switch (*fmt) {
			case 'd':
				unum = (unsigned int)get_num_va_args(args, 0);
				count += print_decimal_num(unum, 10, padc, padn);
				break;
			case 'x':
				unum = (unsigned int)get_unum_va_args(args, 0);
				count += print_decimal_num(unum, 16, padc, padn);
				break;
			case 's':
				str = va_arg(args, char *);
				char *tempstr = str;
...
/* only support %s %d %x, num must within 31bits */
