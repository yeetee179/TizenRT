...
int ble_rmc_main(int argc, char *argv[])
{
	...
	*/
	...
	if (strncmp(argv[1], "server", 7) == 0) {
		...
		if (argc == 6 && strncmp(argv[2], "cremuladv", 10) == 0) {

			ble_conn_handle con_handle = 24;
			uint8_t bd_addr[BLE_BD_ADDR_MAX_LEN];
			ble_server_get_mac_addr_by_conn_handle(con_handle, bd_addr);
			...
			// uint8_t addrr [6] ={0};  
			// if (argc > 2) {   
			// 	adv_event_prop = atoi(argv[3]);  
			// 	own_addr_type = atoi(argv[4]);  
			// 	if (own_addr_type == 1) {
			// 		hexdata_str_to_bd_addr2(argv[5], addrr, 6);  
			// 	}
			// } else { 
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////   
		...
		if (argc == 4 && strncmp(argv[2], "delmuladv", 10) == 0) {  
			...
			uint8_t bd_addr[BLE_BD_ADDR_MAX_LEN] = {0x9c,0x44,0x3d,0x59,0x74,0xe3};
			// bd_addr[5] = 0xe3;  
			// bd_addr[1] = 0x74;
			// bd_addr[2] = 0x59;
			// bd_addr[3] = 0x3d;
			...
			ble_server_get_conn_handle_by_addr(bd_addr, &con_handle);
			// uint8_t adv_handle = 0;   
			// if (argc > 2) {  
