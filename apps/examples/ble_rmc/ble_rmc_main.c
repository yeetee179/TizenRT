...
static void ble_peri_cb_charact_ota(ble_server_attr_cb_type_e type, ble_conn_handle conn_handle, ble_attr_handle attr_handle, void* arg, uint16_t result, uint16_t pending) {  
	char *arg_str = "None";  
	if (arg != NULL) {  
		arg_str = (char *)arg;  
	}  
	if (result){  
		printf("[######## %s : %d] Indication failed\n", __FUNCTION__, __LINE__);  
	}  
	else{  
		printf("[######## %s : %d] Indication Ok\n", __FUNCTION__, __LINE__);  
	}  
	RMC_LOG(RMC_SERVER_TAG, "[CHAR_OTA][%s] type : %d / handle : %d / attr : %02x / result : %02x / pending : %02x \n", arg_str, type, conn_handle, attr_handle, result, pending);  
}  
...
static void gatt_cb(ble_server_attr_cb_type_e type, ble_conn_handle conn_handle, ble_attr_handle attr_handle, void *arg)  
{  
	char *arg_str = "None";  
	if (arg != NULL) {  
		arg_str = (char *)arg;  
...
#define BLE_TEST_A_SERVICE (0x006b) 
#define BLE_TEST_A_COMMAND (0x006c) 
#define BLE_TEST_A_CCCD (0x006e) 
  
#define BLE_TEST_B_SERVICE (0xcc00) 
#define BLE_TEST_B_COMMAND (0xcc01) 
#define BLE_TEST_B_CCCD (0xcc03) 
  
#define BLE_TEST_C_SERVICE (0xff00) 
#define BLE_TEST_C_COMMAND (0xff02) 
#define BLE_TEST_C_CCCD (0xff03) 
...
static ble_server_gatt_t gatt_profile[] = {    
	...
	{    
		...
		.uuid_length = 16,    
		.property =  BLE_ATTR_PROP_READ |BLE_ATTR_PROP_INDICATE|BLE_ATTR_PROP_NOTIFY,
		.permission = BLE_ATTR_PERM_R_PERMIT|BLE_ATTR_PERM_W_PERMIT,    
		.attr_handle = BLE_TEST_A_COMMAND,    
		.cb = gatt_cb,    
		.arg = "BLE_TEST_A_COMMAND"    
	},    
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
	...
	{    
		...
		.uuid_length = 16,    
		.property =  BLE_ATTR_PROP_READ |BLE_ATTR_PROP_WRITE |BLE_ATTR_PROP_INDICATE|BLE_ATTR_PROP_NOTIFY,
		.permission = BLE_ATTR_PERM_R_PERMIT|BLE_ATTR_PERM_W_PERMIT,  
		.attr_handle = BLE_TEST_B_COMMAND,     
		.cb = gatt_cb,     
		.arg = "BLE_TEST_B_COMMAND"    
	},    
	...
	{    
		...
		.permission = BLE_ATTR_PERM_R_PERMIT|BLE_ATTR_PERM_W_PERMIT,    
		.attr_handle = BLE_TEST_C_COMMAND,	   
		.cb = gatt_cb,	   
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////  
	if (strncmp(argv[1], "noti", 5) == 0) {
		...
		ret = ble_server_charact_notify(BLE_TEST_A_COMMAND, con_handle, &packet);  
		if (ret != BLE_MANAGER_SUCCESS) {  
	...
	if (strncmp(argv[1], "indi", 5) == 0) {  
		...
		ret = ble_server_charact_indicate(BLE_TEST_C_COMMAND, con_handle, &packet);  
		if (ret != BLE_MANAGER_SUCCESS) {  
	...
	if (strncmp(argv[1], "read", 5) == 0) {  
		...
		bt_data3.data = data_arr3;  
		bt_data3.length = 1;  
		ret = ble_client_operation_read(bt_ctx, 0x0014, &bt_data3);  
		if (ret != BLE_MANAGER_SUCCESS) {  
			RMC_LOG(RMC_CLIENT_TAG, "Fail to read\n", ret);  
		} else {  
			RMC_LOG(RMC_CLIENT_TAG, "Success to read.\n");  
	...
	if (strncmp(argv[1], "read_bas", 10) == 0) {   
		ble_data bt_data3;  
		uint8_t data_arr3[1] = {0};  
		bt_data3.data = data_arr3;  
		bt_data3.length = 1;  
		ret = ble_client_operation_read(bt_ctx, 0x001a, &bt_data3);  
		if (ret != BLE_MANAGER_SUCCESS) {  
			RMC_LOG(RMC_CLIENT_TAG, "Fail to read\n", ret);  
		} else {  
			RMC_LOG(RMC_CLIENT_TAG, "Success to read.\n");  
	...
	if (strncmp(argv[1], "noti_dis", 9) == 0) {  
		...
		bt_data3.data = data_arr3;  
		bt_data3.length = 1;  
		ret = ble_client_operation_write(bt_ctx, 0x0014, &bt_data3);  
		if (ret != BLE_MANAGER_SUCCESS) {  
			RMC_LOG(RMC_CLIENT_TAG, "Fail to write\n", ret);  
		} else {  
			RMC_LOG(RMC_CLIENT_TAG, "Success to write.\n");  
	...
	if (strncmp(argv[1], "wrt", 4) == 0) {  
		...
		bt_data3.length = 1;  
		ret = ble_client_operation_write(bt_ctx, attr_handle, &bt_data3);  
		if (ret != BLE_MANAGER_SUCCESS) {  
			RMC_LOG(RMC_CLIENT_TAG, "Fail to write\n", ret);  
		} else {  
			RMC_LOG(RMC_CLIENT_TAG, "Success to write.\n");  
