...
static void ble_operation_write_cb(ble_client_ctx *ctx, ble_attr_handle attr_handle, uint8_t result)
{
	printf("[######## %s : %d]conn_handle %d attr_handle %x result %x\n", __FUNCTION__, __LINE__, ctx->conn_handle, attr_handle, result);
	return;
}
...
static void char_cb(ble_server_attr_cb_type_e type, ble_conn_handle conn_handle, ble_attr_handle attr_handle, void *arg)
{
	char *arg_str = "None";
	if (arg != NULL) {
		arg_str = (char *)arg;
...
static void cccd_cb(ble_server_attr_cb_type_e type, ble_conn_handle conn_handle, ble_attr_handle attr_handle, void *arg)
{
	char *arg_str = "None";
	if (arg != NULL) {
		arg_str = (char *)arg;
...
#define BLE_TEST_A_SERVICE (0x006b) //107
#define BLE_TEST_A_CHAR    (0x006c) //108
#define BLE_TEST_A_CCCD    (0x006e) //110
  
#define BLE_TEST_B_SERVICE (0x007b) //123
#define BLE_TEST_B_CHAR    (0x007c) //124
#define BLE_TEST_B_CCCD    (0x007e) //126
  
#define BLE_TEST_C_SERVICE (0x008b) //139
#define BLE_TEST_C_CHAR    (0x008c) //140
#define BLE_TEST_C_CCCD    (0x008e) //142
...
static ble_server_gatt_t gatt_profile[] = {
	{
		...
		.uuid_length = 16,
		.attr_handle = BLE_TEST_A_SERVICE,
	},
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
	{    
		...
		.uuid_length = 16,    
		.attr_handle = BLE_TEST_B_SERVICE,
	},
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
	{
		...
		.uuid_length = 16,
		.attr_handle = BLE_TEST_C_SERVICE,
	},

	{
		...
		.permission = BLE_ATTR_PERM_R_PERMIT|BLE_ATTR_PERM_W_PERMIT,
		.attr_handle = BLE_TEST_C_CHAR,
		.cb = char_cb,
		.arg = "BLE_TEST_C_COMMAND"
	},
...
int ble_rmc_main(int argc, char *argv[])
{
	...
	*/
	...
	if (strncmp(argv[1], "noti_en", 8) == 0) {
		int attr_handle = BLE_TEST_A_CCCD;
		if (argc > 3) {
			bt_ctx->conn_handle = atoi(argv[2]);
			attr_handle = atoi(argv[3]);
		}
		printf("[######## %s : %d] attr_handle %d bt_ctx.conn_handle  %d\n", 
		__FUNCTION__, __LINE__, attr_handle, bt_ctx->conn_handle);

		ret = ble_client_operation_enable_notification(bt_ctx, attr_handle);
		if (ret != BLE_MANAGER_SUCCESS) {
			RMC_LOG(RMC_CLIENT_TAG, "Fail to enable noti handle1[%d]\n", ret);
		} else {
			RMC_LOG(RMC_CLIENT_TAG, "Success to enable noti handle1.\n");
	...
	if (strncmp(argv[1], "indi_en", 8) == 0) {
		int attr_handle = BLE_TEST_B_CCCD;
		if (argc > 3) {
			bt_ctx->conn_handle = atoi(argv[2]);
			attr_handle = atoi(argv[3]);
		}
		printf("[######## %s : %d] attr_handle %d bt_ctx.conn_handle  %d\n", 
		__FUNCTION__, __LINE__, attr_handle, bt_ctx->conn_handle);
		ret = ble_client_operation_enable_indication(bt_ctx, attr_handle);
		if (ret != BLE_MANAGER_SUCCESS) {
			RMC_LOG(RMC_CLIENT_TAG, "Fail to enable noti handle1[%d]\n", ret);
		} else {
			RMC_LOG(RMC_CLIENT_TAG, "Success to enable noti handle1.\n");
	...
	if (strncmp(argv[1], "noti", 5) == 0) {
		int attr_handle = BLE_TEST_A_CCCD;
		ble_conn_handle con_handle = 0;
		if (argc >= 3) {
			con_handle = atoi(argv[2]);
			attr_handle = atoi(argv[3]);
		} else {
		...
		printf("[######## %s : %d] attr_handle %d bt_ctx.conn_handle  %d\n", 
		__FUNCTION__, __LINE__, attr_handle, con_handle);
		...
		ret = ble_server_charact_notify(attr_handle, con_handle, &packet);
		if (ret != BLE_MANAGER_SUCCESS) {
	...
	if (strncmp(argv[1], "indi", 5) == 0) {
		...
		ret = ble_server_charact_indicate(attr_handle, con_handle, &packet);
		if (ret != BLE_MANAGER_SUCCESS) {
	...
	if (strncmp(argv[1], "indiq", 6) == 0) {
		ble_conn_handle con_handle;
		if (argc > 2) {
			con_handle = atoi(argv[2]);
		}
		else{
		...
		uint8_t ind_count[1] = {0xff};
		int ret = ble_server_get_indicate_pending_count(con_handle, ind_count);
		printf("[######## %s : %d] ind_count %d\n", __FUNCTION__, __LINE__, ind_count[0]);
		if (ret != BLE_MANAGER_SUCCESS) {
			RMC_LOG(RMC_SERVER_TAG, "Get indicate queue fail[%d]\n", ret);
		} else {
			RMC_LOG(RMC_SERVER_TAG, "Get indicate queue OK\n");
		}
	}
	...
	if (strncmp(argv[1], "read", 5) == 0) {
		...
		printf("[######## %s : %d] attr_handle %d conn_handle  %d\n", 
		__FUNCTION__, __LINE__, attr_handle, bt_ctx->conn_handle);

		ret = ble_client_operation_read(bt_ctx, attr_handle, &bt_data3);
		if (ret != BLE_MANAGER_SUCCESS) {
	...
	if (strncmp(argv[1], "wrtA", 5) == 0) {
		ble_data bt_data3;
		uint8_t data_arr3[2] = {11};
		bt_data3.data = data_arr3;
		bt_data3.length = 2;

		ble_attr_handle attr_handle = 0;
		// if (argc > 3) {
		// 	bt_ctx->conn_handle = atoi(argv[2]);
		// 	attr_handle = atoi(argv[3]);
		// }
		...
		ret = ble_client_operation_write(bt_ctx, BLE_TEST_A_CHAR+1, &bt_data3);
	...
	if (strncmp(argv[1], "wrtB", 5) == 0) {
		...
		ret = ble_client_operation_write(bt_ctx, BLE_TEST_B_CHAR+1, &bt_data3);
...
if (strncmp(argv[1], "wrtX", 5) == 0) {
		...
		ret = ble_client_operation_write(bt_ctx, 255, &bt_data3);
  	...
  	if (strncmp(argv[1], "wrt_noA", 7) == 0) {
		...
		ret = ble_client_operation_write_no_response(bt_ctx, BLE_TEST_A_CHAR+1, &bt_data3);
		if (ret != BLE_MANAGER_SUCCESS) {
			RMC_LOG(RMC_CLIENT_TAG, "Fail to write\n", ret);
		} else {
			RMC_LOG(RMC_CLIENT_TAG, "Success to write.\n");
	...
	if (strncmp(argv[1], "wrt_noB", 7) == 0) {
		...
		ret = ble_client_operation_write_no_response(bt_ctx, BLE_TEST_B_CHAR+1, &bt_data3);
		if (ret != BLE_MANAGER_SUCCESS) {
			RMC_LOG(RMC_CLIENT_TAG, "Fail to write\n", ret);
		} else {
			RMC_LOG(RMC_CLIENT_TAG, "Success to write.\n");
